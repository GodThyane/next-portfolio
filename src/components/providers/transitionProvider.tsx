...
const TransitionProvider = ({
    ...
    const formatName = useMemo(() => {
        // @ts-ignore
        return dictionaryLinks[pathName] || "José Dev";
    }, [pathName]);
    ...
    return (
        <AnimatePresence mode="wait">
            <div key={pathName} className="w-screen h-screen bg-gradient-to-b from-blue-100 to-red-100">
                ...
                />
                <motion.div
                    className="fixed m-auto top-0 bottom-0 left-0 right-0 text-white text-5xl sm:text-8xl cursor-default z-50 w-fit h-fit"
                    initial={{opacity: 1}}
                    animate={{opacity: 0}}
                    exit={{opacity: 0}}
